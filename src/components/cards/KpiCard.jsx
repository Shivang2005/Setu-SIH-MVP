  ...
  return (
    <Card className="flex flex-col gap-3 min-w-0">
      <div className="flex items-start justify-between">
        ...
        {Icon && (
          <span className={`w-8 h-8 rounded-lg flex items-center justify-center ${accent ? "bg-accent-50 text-accent-600" : "bg-slate-100 text-slate-500"}`}>
            <Icon className="w-4 h-4" />
          </span>
        )}
      </div>
      <div className="flex items-end justify-between">
        <span className="font-mono tabular text-3xl font-semibold text-ink-900 truncate">
          {value}
          {suffix}
        </span>
        {!isFlat && (
